...
int main() {
	...
	for (int i = 0; i < N * N; ++i) {	// N^2 학생에 대해 진행
		...
		for (int j = 0; j < 4; ++j) {	// 좋아하는 자리 체크
			...
			if (Pos[Like[j]][0] == 0 && Pos[Like[j]][1] == 0) continue;	// 자리 배정 아직 안됐으면 스킵
		...
		Map[Fav.first][Fav.second] = std;	// 자리 배정
		Pos[std][0] = Fav.first;	// 학생 위치 저장

/*
주의사항!
1. 거리가 같으면 행 작은순, 같으면 열 작은순!
2. 시작위치랑 택시좌표는 1부터 시작!!
...
struct node
{
    ...
    {
        if (nowdist < next.nowdist)
            return true;
        if (nowdist > next.nowdist)
            return false;
        if (start_y < next.start_y)
            return true;
        if (start_y > next.start_y)
            return false;
...
{ // 출발지에서 도착지까지 거리 구하기
    ...
    SolveMap[now.start_y][now.start_x] = 1; // 시작위치
    while (!q.empty())
    {
        node2 Now = q.front();
        q.pop();
    ...
    return SolveMap[now.end_y][now.end_x] - 1; // 시작을 1로 했었어서
}
...
int FindShort(node2 now)
{
    ...
    for (int i = 0; i < M; ++i)
    {
        if (Texi[i].dist == -1)
            continue; // 이미 완료한 택시
        if (SolveMap[Texi[i].start_y][Texi[i].start_x] == 0)
            continue;                                                     // 갈 수 없는 곳이면
        Texi[i].nowdist = SolveMap[Texi[i].start_y][Texi[i].start_x] - 1; // 택시까지의 거리
        v.push_back(Texi[i]);
    }
    sort(v.begin(), v.end());

    if (v.size() == 0)
        return -1; // 갈 수 있는 택시가 없으면 -1 리턴
    else
    {
...
int main()
{
    ...
    for (int i = 0; i < M; ++i)
    {
        ...
        int NextTexi = FindShort(StartPos); // 가장 가까운 택시 번호
        if (NextTexi == -1)
        { // 갈 수 있는 택시가 없으면 종료
            flag = false;
            break;
        ...
        Fuel = Fuel - Texi[NextTexi].nowdist - Texi[NextTexi].dist; // 기름 계산
        if (Fuel < 0)
        { // 기름이 다 떨어지면 종료
            flag = false;
            break;
        }
        Fuel += Texi[NextTexi].dist * 2; // 기름 충전
        Texi[NextTexi].dist = -1;        // 방문 택시 체크
        StartPos = {Texi[NextTexi].end_y, Texi[NextTexi].end_x};
    }

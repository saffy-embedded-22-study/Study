...
void solve() {
	...
	while (!pq.empty()) {
		...
		for (int dir = 0; dir < 4; dir++) {
			...
			// 그냥 등호를 없애면 같을 경우 중복이 너무 많이 생겨서 터짐!!

			if (Visited[dy][dx] < nextcost) continue;	 
			else if (Visited[dy][dx] == nextcost) {	// 같을 경우에 대해 방향 정보를 저장해둬서 중복 방지해주기.
				if (DirVisited[dy][dx][dir] == 1)continue;
				DirVisited[dy][dx][dir] = 1;
			}
			else {
				for (int k = 0; k < 4; k++) {	// 작은 값이 나올경우 방향 정보 초기화
